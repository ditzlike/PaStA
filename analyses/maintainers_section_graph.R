...
if (length(args) == 0) {
  file_name <- 'resources/linux/resources/section_graph.csv'
} else {
  file_name <- args[2]
}
...
data_frame$weight <- as.numeric(data_frame$lines)
...
# best used with low quantiles for vertices and edges
linder_edge_density <- function(param){
  
  for (v in V(param)){
    vertex_edges <- E(param)[from(v)]
    quan <- quantile(vertex_edges$weight, probs = probes)
    victims <- which(vertex_edges$weight < quan['10%'])
    
    param <- igraph::delete.edges(param, victims)
  }
  
  return(layout.fruchterman.reingold(param, niter = 500))
}

# deleting the vast majority of all edges, to only keep the most essential ones
# for the layout
layout_strongest_edges_only <- function(param){
  
  # generating a new graph and assigning edge weights based on groups
  edge_quantiles <- quantile(E(param)$weight, probs=probes)
  
  g_grouped <- param
  g_grouped <- igraph::delete.edges(param, which(E(param)$weight
                                             < unname(edge_quantiles[EDGE_QUANTILE])))
...
if (PRINT_ENTIRE_GRAPH) {
  ...
  plot_file_name <- paste0(d_dst, '/', 'complete_graph')
  ...
  printplot(plot, plot_file_name, 4.5)
}

if(ABBREVIATE){
  for(i in seq(1, length(V(g)), 1)) {
    #abbrev <- paste0(substr(V(g)[i]$name, 1, 40), "...")
    abbrev <- gsub("[AEIOUaeiou]", "", V(g)[i]$name)
    g <- igraph::set.vertex.attribute(g, "name", i, abbrev)
  }
}
...
if(PRINT_CLUSTERS){
  ...
  for (i in bounds){
    ...
    plot_file_name <- paste0(d_dst, '/', toString(i))
    ...
    plot <- plot.igraph(cluster_graph,
                ...
                mark.col = PALETTE,
                vertex.size=VERTEX_SIZE,
                #vertex.label = V(cluster_graph)$abbrev,
                vertex.label.family = FONT_FAMILY,
                vertex.label.dist=0.5,
                vertex.label.cex=LABEL_SIZE,
